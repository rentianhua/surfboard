...
namespace Cedar.Framwork.Caching.Configuration
{
    ...
    public class CachingSettings : ServiceLocatableSettings
    {
        ...
        public NameTypeConfigurationElementCollection<CachingData> CachingProviders
        {
            get
            {
                return (NameTypeConfigurationElementCollection<CachingData>)base["cachingProviders"];
            }
            set
            {
                base["cachingProviders"] = value;
            }
        }
        ...
        public override void Configure(IServiceLocator serviceLocator)
        {
            ...
            foreach (CachingData cachingData in this.CachingProviders)
            {
                Func<CachingProviderBase> providerCreator = cachingData.GetProviderCreator(this);
                if (providerCreator != null)
                {
                    serviceLocator.Register<CachingProviderBase>(providerCreator, cachingData.Name,
                        cachingData.Name == this.DefaultProvider, cachingData.Lifetime);
                }
            }
        }
    }
