...
namespace Cedar.Core.EntLib.IoC
{
    ...
    public class AutoInterceptionStrategy : BuilderStrategy
    {
        ...
        public override void PreBuildUp(IBuilderContext context)
        {
            ...
            var policy = AutoInterceptionStrategy.FindInterceptionPolicy<IInstanceInterceptionPolicy>(context, true);

            if (policy == null)
            {
                AutoInterceptorPolicy autoInterceptorPolicy = context.NewBuildUp<FixedAutoInterceptorPolicy>(typeof(AutoInterceptorPolicy).AssemblyQualifiedName);
                if (autoInterceptorPolicy == null)
                {
                    return;
                }

                IInstanceInterceptor interceptor = autoInterceptorPolicy.Interceptor;
                if (interceptor == null)
                {
                    return;
                }
                ...
                context.Policies.Set<IInstanceInterceptionPolicy>(new FixedInstanceInterceptionPolicy(interceptor), context.BuildKey.Type);
                context.Policies.Clear<IInterceptionBehaviorsPolicy>(context.BuildKey);
            }
            if (AutoInterceptionStrategy.FindInterceptionPolicy<IInterceptionBehaviorsPolicy>(context, true) == null)
            {
                var interceptionBehavior = new InterceptionBehavior<PolicyInjectionBehavior>();
                interceptionBehavior.AddPolicies(context.OriginalBuildKey.Type, context.BuildKey.Type, context.BuildKey.Name, context.Policies);
            }
        }
        ...
        {
            Guard.ArgumentNotNull(context, "context");
            return !context.BuildKey.Type.IsTypeOfUnity() && !context.BuildKey.Type.IsTypeOfEntLib() && !typeof(AutoInterceptorPolicy).IsAssignableFrom(context.BuildKey.Type);
        }
