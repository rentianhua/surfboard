...
namespace Senparc.Weixin.Containers
{
    ...
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public abstract class RedisBaseContainer<T> where T : IBaseContainerBag, new()
    {
        /// <summary>
        /// 所有数据集合的列表
        /// </summary>
        private readonly Dictionary<Type, Dictionary<string, T>> _collectionList = new Dictionary<Type, Dictionary<string, T>>();

        private readonly RedisDatabaseWrapper _wraper;
        ...
        public RedisBaseContainer(string ip, int database, string password, int port) : base()
        {
            _wraper = new RedisDatabaseWrapper(ip, database, password, port);
        }
